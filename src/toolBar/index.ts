...
export const g_ToolbarKeyTextDraw = new SafetyMap<number, TextDraw | null>(() => null);
...
PlayerEvent.onStateChange(({ player, newState, oldState, next }) => {
  if (newState === PlayerStateEnum.SPECTATING || oldState === PlayerStateEnum.SPECTATING) {
...
TextDrawEvent.onPlayerClickGlobal(({ player, textDraw, next }) => {
  if (textDraw === InvalidEnum.TEXT_DRAW) {
    ...
    Object.values(g_ToolbarTextDraw).forEach((gtd: ITdToolBar[keyof ITdToolBar]) => {
      if (gtd && gtd.isValid()) {
        gtd.hide(player);
      }
    });
  ...
  if (textDraw === g_ToolbarTextDraw.objectLookSelect) {
    ...
    if (!object || object.id === InvalidEnum.OBJECT_ID) {
      player.sendClientMessage(RGBA_RED, "ERROR: You are not looking at any object!");
      return 1;
    }
  ...
  if (textDraw === g_ToolbarTextDraw.objectSSelect) {
    ...
    if (!object || object.id === InvalidEnum.OBJECT_ID) {
      player.sendClientMessage(RGBA_RED, "ERROR: You are not surfing any object!");
      return 1;
    }
  ...
  if (textDraw === g_ToolbarTextDraw.vehicleNSelect) {
    ...
    if (vehicleId === InvalidEnum.VEHICLE_ID) {
      player.sendClientMessage(RGBA_RED, "ERROR: You are not near any vehicle!");
      return 1;
    }
  ...
  if (textDraw === g_ToolbarTextDraw.vehicleLookSelect) {
    ...
    if (!vehicle || vehicle.id === InvalidEnum.VEHICLE_ID) {
      player.sendClientMessage(RGBA_RED, "ERROR: You are not looking at any vehicle!");
      return 1;
    }
  ...
  if (textDraw === g_ToolbarTextDraw.vehicleSSelect) {
    ...
    if (!vehicle || vehicle.id === InvalidEnum.VEHICLE_ID) {
      player.sendClientMessage(RGBA_RED, "ERROR: You are not surfing at any vehicle!");
      return 1;
    }
  ...
  if (textDraw === g_ToolbarTextDraw.actorLookSelect) {
    ...
    if (!actor || actor.id === InvalidEnum.ACTOR_ID) {
      player.sendClientMessage(RGBA_RED, "ERROR: You are not looking at any actor!");
      return 1;
    }
