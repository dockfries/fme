...
import { defaultPlayerAttachData, g_PlayerAttachData, MAX_PLAYERATTACH_INDEX } from "@/pAttach";
  ...
  switch (dialogId) {
    case DIALOG_ID.MAP_NEW: {
      ...
      if (!inputText.trim().length || inputText !== NEWMAP_COMMAND) {
        ...
        let g_ClientMessage = `[${player.id}] ${player.getName().name} has started a new map.`;
...
export function newMap(player: Player | InvalidEnum.PLAYER_ID = InvalidEnum.PLAYER_ID) {
