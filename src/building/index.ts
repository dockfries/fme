...
export function loadBuildingCache() {
  ...
  for (let row = 0; row < rows; row++) {
    ...
    if (isValidBuildingID(buildingId) && !g_BuildingData.get(buildingId).isLoaded) {
  ...
  if (buildingsLoaded !== BUILDING_DATA_SIZE) {
    logger.error(`error: ${buildingsLoaded} / ${BUILDING_DATA_SIZE} buildings were loaded!`);
  }
}
  ...
  for (let b = 0; b < BUILDING_DATA_SIZE; b++) {
    ...
    if (
      (incMode === BUILDING_INC_MODE.ALL ||
        (incMode === BUILDING_INC_MODE.REMOVED && g_BuildingData.get(b).isRemoved) ||
        (incMode === BUILDING_INC_MODE.EXISTING && !g_BuildingData.get(b).isRemoved)) &&
