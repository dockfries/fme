import { INVALID_COLOR_ID, INVALID_ROW, RGBA_RED, RGBA_WHITE } from "@/constants";
import { DIALOG_ID } from "@/dialog";
import { MATERIAL_INDEX_TYPE } from "@/matIndexType";
import { findModelColors, getModelColorName, getModelColorRGB } from "@/modelColor";
import {
...
import { findVehicleColors, getVehicleColorRGB, getVehicleColorName } from "@/vehColor";
    ...
    if (
      ...
      switch (g_PlayerData.get(player.id).tdMode) {
        case TD_MODE.COLORLIST_TEXTURE: {
          ...
          g_ObjectData.get(objectId - 1).matIndexColor[materialIndex] = colorArgb;
        ...
        case TD_MODE.COLORLIST_FONTFACE: {
          ...
          g_ObjectData.get(objectId - 1).matIndexFontColor[materialIndex] = colorArgb;
...
export function destroyPlayerColorList(player: Player) {
  Object.entries(g_ColorListPTD.get(player.id)).forEach((item) => {
    const [key, ptd] = item as unknown as [keyof IColorListPtd, IColorListPtd[keyof IColorListPtd]];
...
export function showColorList(player: Player) {
  ...
  Object.values(g_ColorListGTD).forEach((gtd: IColorListGtd[keyof IColorListGtd]) => {
    if (gtd && gtd.isValid()) {
      gtd.show(player);
    }
  });
...
export function getColorListNewAppliedRow(player: Player) {
  switch (g_PlayerData.get(player.id).tdMode) {
    case TD_MODE.COLORLIST_TEXTURE: {
      ...
      const materialIndex = g_PlayerData.get(player.id).editObjMatIdx;
      if (
        g_ObjectData.get(objectId - 1).matIndexType[materialIndex] !== MATERIAL_INDEX_TYPE.TEXTURE
      ) {
        return INVALID_ROW;
      }

      const objColorArgb = g_ObjectData.get(objectId - 1).matIndexColor[materialIndex];
    ...
    case TD_MODE.COLORLIST_FONTFACE: {
      ...
      const objColorArgb = g_ObjectData.get(objectId - 1).matIndexFontColor[materialIndex];
      const objColorRgb = aRGBtoRGB(objColorArgb);
    ...
    case TD_MODE.COLORLIST_ATTACH_1: {
      ...
      const attachColorArgb = g_PlayerAttachData.get(player.id)[attachIndex].color1;
    ...
    case TD_MODE.COLORLIST_ATTACH_2: {
      ...
      const attachColorArgb = g_PlayerAttachData.get(player.id)[attachIndex].color2;
      const attachColorRgb = aRGBtoRGB(attachColorArgb);
...
export function loadColorListRowData(player: Player) {
  ...
  switch (g_PlayerData.get(player.id).tdMode) {
    ...
    case TD_MODE.COLORLIST_ATTACH_2: {
      ...
      g_ModelColorListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_COLORLIST_ROWS);

      g_ModelColorListData.get(player.id).appliedRow = getColorListNewAppliedRow(player);
      break;
    }
    case TD_MODE.COLORLIST_VEHICLE_1:
    case TD_MODE.COLORLIST_VEHICLE_2: {
      ...
      g_VehColorListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_COLORLIST_ROWS);

      g_VehColorListData.get(player.id).appliedRow = getColorListNewAppliedRow(player);
      break;
    }
    default: {
      return 0;
