...
export const g_BuildListData = new SafetyMap<number, IBuildListData>(() => {
  return {
    ...
    incMode: 0,
    rowId: Array.from({ length: MAX_BUILDLIST_ROWS }, () => INVALID_BUILDING_ID),
    editRow: INVALID_ROW,
    editViewed: false,
  };
});
  ...
  for (let row = 0; row < MAX_BUILDLIST_ROWS; row++) {
    if (
      ...
      if (row !== prevRow) {
        ...
        g_ModelViewData.get(player.id).modelId = g_BuildingData.get(buildingId).model;
...
export function destroyPlayerBuildList(player: Player) {
  Object.entries(g_BuildListPTD.get(player.id)).forEach((item) => {
    const [key, ptd] = item as unknown as [keyof IBuildListPtd, IBuildListPtd[keyof IBuildListPtd]];
...
export function showBuildList(player: Player) {
  ...
  Object.values(g_BuildListGTD).forEach((gtd: IBuildListGtd[keyof IBuildListGtd]) => {
    if (gtd && gtd.isValid()) {
      gtd.show(player);
    }
  });
  ...
  g_ModelViewData.get(player.id).modelId =
    buildingId === INVALID_BUILDING_ID ? INVALID_MODEL_ID : g_BuildingData.get(buildingId).model;
...
export function applyBuildListIncludeMode(player: Player) {
  switch (g_BuildListData.get(player.id).incMode) {
    case BUILDING_INC_MODE.ALL: {
      g_BuildListPTD.get(player.id).incMode?.setString("Including Buildings: All");
      break;
    }
    case BUILDING_INC_MODE.REMOVED: {
      g_BuildListPTD.get(player.id).incMode?.setString("Including Buildings: Removed Only");
      break;
    }
    case BUILDING_INC_MODE.EXISTING: {
      g_BuildListPTD.get(player.id).incMode?.setString("Including Buildings: Existing Only");
      break;
    }
    default: {
      g_BuildListPTD.get(player.id).incMode?.setString("Including Buildings: Unknown Mode");
      return 0;
    }
  }
  return 1;
...
export function applyBuildListRowColor(player: Player, row: number) {
  ...
  if (row === g_BuildListData.get(player.id).editRow) {
    rgbaBoxColor = g_BuildListData.get(player.id).editViewed ? 0xffff0064 : 0xffff0032;
  } else {
    rgbaBoxColor = 0x00000000;
  }

  g_BuildListPTD.get(player.id).buildIdRow[row]?.setBoxColors(rgbaBoxColor).show();
  g_BuildListPTD.get(player.id).modelIdRow[row]?.setBoxColors(rgbaBoxColor).show();
  g_BuildListPTD.get(player.id).modelNameRow[row]?.setBoxColors(rgbaBoxColor).show();
  return 1;
}
