...
import { createToolbarKeyTextDraw, g_ToolbarTextDraw, ITdToolBar } from "@/toolBar";
...
function hook_SelectTextDraw(playerId: number, hoverColor: number | string) {
  ...
  const ret = orig_SelectTextDraw(playerId, hoverColor);
...
function hook_CancelSelectTextDraw(playerId: number) {
  ...
  const ret = orig_CancelSelectTextDraw(playerId);
