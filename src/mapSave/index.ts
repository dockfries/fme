...
import { INVALID_MAP_ID, MAX_OBJECT_INDEX, RGBA_RED, RGBA_WHITE } from "@/constants";
...
function onDialogResponse(player: Player, res: IDialogResCommon, dialogId: number) {
  ...
  switch (dialogId) {
    case DIALOG_ID.MAP_SAVE_AS: {
      ...
      if (!saveSuccess) {
        player.sendClientMessage(RGBA_RED, "ERROR: This map could not be saved!");
    ...
    case DIALOG_ID.MAP_SAVE_CONFIRM: {
      ...
      let g_ClientMessage = `[${player.id}] ${player.getName().name} has saved the map.`;
      Player.sendClientMessageToAll(RGBA_WHITE, g_ClientMessage);
...
export function saveMap(
    ...
    for (let materialIndex = 0; materialIndex < MAX_OBJECT_INDEX; materialIndex++) {
      switch (g_ObjectData.get(objId - 1).matIndexType[materialIndex]) {
        case MATERIAL_INDEX_TYPE.TEXTURE: {
          const textureId = g_ObjectData.get(objId - 1).matIndexTexture[materialIndex];
        ...
        case MATERIAL_INDEX_TYPE.TEXT: {
          ...
          const matIndexColor = g_ObjectData
            ...
            .matIndexColor[materialIndex].toString(16)
            .padStart(8, "0");
          const matIndexAlign = g_ObjectData.get(objId - 1).matIndexAlignment[materialIndex];
  ...
  for (let a = 0; a < slotActorId.length; a++) {
    ...
    if (animIndex != INVALID_ANIM_INDEX) {
      ...
      const writeString =
        ...
        `${g_ActorData.get(actorId).animLockX}, ${g_ActorData.get(actorId).animLockY}, ` +
  ...
  if (player !== InvalidEnum.PLAYER_ID) {
    ...
    for (let index = 0; index < MAX_PLAYERATTACH_INDEX; index++) {
      ...
      const g_ModelString = getModelName(g_PlayerAttachData.get(player.id)[index].model).name;
      const g_BoneString = getBoneName(g_PlayerAttachData.get(player.id)[index].bone);
  ...
  for (let b = 0; b < BUILDING_DATA_SIZE; b++) {
    ...
    const writeString =
      ...
      `${g_BuildingData.get(b).model}, ${g_BuildingData.get(b).x.toFixed(4)}, ` +
