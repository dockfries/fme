...
export const g_AnimListData = new SafetyMap<number, IAnimListData>(() => {
  return {
    ...
    maxPage: 0,
    search: "",
    rowIndex: Array.from({ length: MAX_ANIM_LIST_ROWS }, () => INVALID_ANIM_INDEX),
    coloredRow: 0,
  };
});
...
function onDialogResponse(player: Player, res: IDialogResCommon, dialogId: number) {
  ...
  switch (dialogId) {
    case DIALOG_ID.ANIMLIST_PAGE: {
      ...
      if (page < MIN_ANIM_LIST_PAGE || page > g_AnimListData.get(player.id).maxPage) {
        player.sendClientMessage(RGBA_RED, "ERROR: You did not enter a valid page number!");
        showAnimationListDialog(player, dialogId);
        return 1;
      }
...
TextDrawEvent.onPlayerClickGlobal(({ player, textDraw, next }) => {
  ...
  if (textDraw === g_AnimListGTD.pageN) {
    if (g_AnimListData.get(player.id).page === g_AnimListData.get(player.id).maxPage) {
      return 1;
    }
...
export function destroyPlayerAnimationList(player: Player) {
  Object.entries(g_AnimListPTD.get(player.id)).forEach((item) => {
    const [key, ptd] = item as unknown as [keyof IAnimListPtd, IAnimListPtd[keyof IAnimListPtd]];
...
export function showAnimationList(player: Player) {
  ...
  Object.values(g_AnimListGTD).forEach((gtd: IAnimListGtd[keyof IAnimListGtd]) => {
    if (gtd && gtd.isValid()) {
      gtd.show(player);
    }
  });
...
export function applyAnimationListRowColor(player: Player, row: number) {
  ...
  g_AnimListPTD.get(player.id).rowLibrary[row]?.setBoxColors(rgbaBoxColor).show();
...
export function loadAnimationListRowData(player: Player) {
  ...
  g_AnimListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_ANIM_LIST_ROWS);

  g_AnimListData.get(player.id).coloredRow = getAnimationListNewColoredRow(player);
}

export async function showAnimationListDialog(player: Player, dialogId: number) {
  let g_DialogInfo = "";
  switch (dialogId) {
    case DIALOG_ID.ANIMLIST_PAGE: {
      const _page = g_AnimListData.get(player.id).page + 1;
