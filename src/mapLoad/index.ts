...
import { applyPlayerAttachData, g_PlayerAttachData, isValidPlayerAttachIndex } from "@/pAttach";
...
async function onDialogResponse(player: Player, res: IDialogResCommon, dialogId: number) {
  ...
  switch (dialogId) {
    case DIALOG_ID.MAP_LOAD: {
      ...
      if (!loadSuccess) {
        player.sendClientMessage(RGBA_RED, "ERROR: This map could not be loaded!");
        showMapLoadDialog(player, dialogId);
        return 1;
      }
...
export function mapLoad_CreateVarName(id: number, idType: number, varName: string) {
  switch (idType) {
    case ID_TYPE.OBJECT: {
  ...
  for await (const line of rl) {
    ...
    if (funcName === "CreateObject") {
      try {
        ...
        const [modelId, x, y, z, rx, ry, rz, drawDistance = 0] = objParams.map((n) => +n || 0);
      ...
      const [modelId, x, y, z, zAngle, color1, color2] = vehParams.map((n) => +n || 0);
      ...
      g_ObjectData.get(objectId - 1).matIndexType[_matIndex] = MATERIAL_INDEX_TYPE.TEXT;
      ...
      g_ObjectData.get(objectId - 1).matIndexIsBold[_matIndex] = Boolean(+isBold);
      ...
      g_ObjectData.get(objectId - 1).matIndexType[_matIndex] = MATERIAL_INDEX_TYPE.TEXTURE;
      ...
      const [index, modelId, bone, x, y, z, rx, ry, rz, sx, sy, sz, color1, color2] =
      ...
      const [modelId, x, y, z, radius] = removeBuildingParams.map((n) => +n || 0);
