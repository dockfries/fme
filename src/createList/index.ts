...
import { findVehicleModels, getVehicleModelName, isValidVehicleModel } from "@/vehModel";
import {
  Actor,
  Dialog,
  DialogStylesEnum,
...
export const g_CreateObjListData = new SafetyMap<number, ICreateListData>(() => {
  return {
    ...
    search: "",
    rowId: Array.from({ length: MAX_CREATELIST_ROWS }, () => INVALID_MODEL_ID),
...
export function showCreateList(player: Player) {
  ...
  Object.values(g_CreateListGTD).forEach((gtd: ICreateListGtd[keyof ICreateListGtd]) => {
    if (gtd && gtd.isValid()) {
      gtd.show(player);
    }
  });
...
export function applyCreateListRowColor(player: Player, row: number) {
  ...
  g_CreateListPTD.get(player.id).nameRow[row]?.setBoxColors(rgbaBoxColor).show();
}
...
export function applyCreateListRowData(player: Player) {
  for (let row = 0; row < MAX_CREATELIST_ROWS; row++) {
    ...
    if (isValidVehicleModel(rowModelId)) {
      ...
      g_CreateListPTD.get(player.id).nameRow[row]?.setString(g_VehModelString || "");
    } else if (isValidSkin(rowModelId)) {
      const g_SkinString = getSkinName(rowModelId);
      g_CreateListPTD.get(player.id).nameRow[row]?.setString(g_SkinString || "");
    } else {
      const g_ModelString = getModelName(rowModelId).name;
      g_CreateListPTD.get(player.id).nameRow[row]?.setString(g_ModelString || "");
    }
...
export function loadCreateListRowData(player: Player) {
  ...
  switch (g_PlayerData.get(player.id).tdMode) {
    case TD_MODE.CREATELIST_OBJECT: {
      ...
      g_CreateObjListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_CREATELIST_ROWS);
    ...
    case TD_MODE.CREATELIST_VEHICLE: {
      ...
      g_CreateVehListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_CREATELIST_ROWS);
    ...
    case TD_MODE.CREATELIST_PICKUP: {
      ...
      g_CreatePickListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_CREATELIST_ROWS);
    ...
    case TD_MODE.CREATELIST_ACTOR: {
      ...
      g_CreateActListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_CREATELIST_ROWS);
    ...
    case TD_MODE.CREATELIST_ATTACH: {
      ...
      g_CreateAttachListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_CREATELIST_ROWS);
