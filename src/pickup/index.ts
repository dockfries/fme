...
import { INVALID_ARRAY_INDEX, INVALID_MODEL_ID, RGBA_ORANGE, RGBA_RED } from "@/constants";
...
import { applySelectListRowData, g_SelectPickListData, MAX_SELECT_LIST_ROWS } from "@/selectList";
...
ObjectMpEvent.onPlayerEdit(({ player, objectMp, isPlayerObject, response, fX, fY, fZ, next }) => {
  const pickupId = getPlayerEditPickup(player);

  if (
    isPlayerObject &&
    objectMp.id === g_PlayerData.get(player.id).clickDragPoId &&
    Pickup.isValidGlobal(pickupId)
  ) {
    switch (response) {
      case EditResponseTypesEnum.FINAL: {
        ...
        if (recreatePickup(pickupId) === INVALID_PICKUP_ID) {
          player.sendClientMessage(RGBA_RED, "ERROR: This pickup could not be moved / re-created");
        }
...
function onDialogResponse(player: Player, res: IDialogResCommon, dialogId: number) {
  ...
  switch (dialogId) {
    case DIALOG_ID.PICKUP_MAIN: {
      const pickupId = getPlayerEditPickup(player);
      if (pickupId === InvalidEnum.PICKUP_ID || !g_PickupData.get(pickupId).isValid) {
      ...
      switch (listItem) {
        ...
        case DIALOG_LISTITEM_PICKUP.MOVE: {
          ...
          if (objectId === InvalidEnum.OBJECT_ID) {
            player.sendClientMessage(RGBA_RED, "ERROR: This pickup cannot be moved right now!");
          } else {
        ...
        case DIALOG_LISTITEM_PICKUP.DUPLICATE: {
          ...
          if (newPickupId === INVALID_PICKUP_ID) {
            player.sendClientMessage(RGBA_RED, "ERROR: This pickup could not be duplicated!");
          } else {
        ...
        case DIALOG_LISTITEM_PICKUP.COMMENT_RESET: {
          const g_CommentString = getModelName(g_PickupData.get(pickupId).model).name;
...
export function isValidPickup(pickupId: number) {
  return pickupId >= 0 && pickupId < LimitsEnum.MAX_PICKUPS && g_PickupData.get(pickupId).isValid;
}
...
export async function showPickupDialog(player: Player, dialogId: number) {
  switch (dialogId) {
    case DIALOG_ID.PICKUP_MAIN: {
      const pickupId = getPlayerEditPickup(player);
      if (!Pickup.isValidGlobal(pickupId)) {
        return 1;
      }

      let g_DialogInfo = "";

      for (let listItem = 0; listItem < DIALOG_LISTITEM_PICKUP.MAX; listItem++) {
        switch (listItem) {
          case DIALOG_LISTITEM_PICKUP.GOTO: {
            g_DialogInfo += "Goto\t \n";
            break;
          ...
          case DIALOG_LISTITEM_PICKUP.COMMENT_RESET: {
            const g_ModelString = getModelName(g_PickupData.get(pickupId).model).name;
...
export function copyPickup(copyPickupId: number) {
  try {
    ...
    if (pastePickupId !== INVALID_PICKUP_ID) {
      ...
      g_PickupData.get(pastePickupId).memoryX = g_PickupData.get(copyPickupId).memoryX;
      g_PickupData.get(pastePickupId).memoryY = g_PickupData.get(copyPickupId).memoryY;
      g_PickupData.get(pastePickupId).memoryZ = g_PickupData.get(copyPickupId).memoryZ;
    }
    return pastePickupId;
  } catch {
...
export function findPickups(result: number[], resultSize: number, search: string, offset: number) {
  let rowsFound = 0;
  let rowsAdded = 0;
  let maxOffset = 0;
