...
function onDialogResponse(player: Player, res: IDialogResCommon, dialogId: number) {
  ...
  switch (dialogId) {
    case DIALOG_ID.FONTLIST_PAGE: {
      ...
      if (page < MIN_FONTLIST_PAGE || page > g_FontListData.get(player.id).maxPage) {
        player.sendClientMessage(RGBA_RED, "ERROR: You did not enter a valid page number!");
        showFontListDialog(player, dialogId);
        return 1;
      }
...
TextDrawEvent.onPlayerClickGlobal(({ player, textDraw, next }) => {
  ...
  if (textDraw === g_FontListGTD.pageN) {
    if (g_FontListData.get(player.id).page == g_FontListData.get(player.id).maxPage) {
      return 1;
    }
...
export function destroyPlayerFontList(player: Player) {
  Object.entries(g_FontListPTD.get(player.id)).forEach((item) => {
    const [key, ptd] = item as unknown as [keyof IFontListPtd, IFontListPtd[keyof IFontListPtd]];
...
export function showFontList(player: Player) {
  ...
  Object.values(g_FontListGTD).forEach((gtd: IFontListGtd[keyof IFontListGtd]) => {
    if (gtd && gtd.isValid()) {
      gtd.show(player);
    }
  });
...
export function loadFontListRowData(player: Player) {
  ...
  g_FontListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_FONTLIST_ROWS);
...
export function getFontListNewColoredRow(player: Player) {
  ...
  for (let row = 0; row < MAX_FONTLIST_ROWS; row++) {
    ...
    if (
      g_FontCache[rowFontId].toLocaleLowerCase() ===
      g_ObjectFont.get(objectId - 1)[materialIndex].toLocaleLowerCase()
    ) {
      return row;
    }
  }
