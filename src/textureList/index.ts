...
import { INVALID_MODEL_ID, INVALID_ROW, RGBA_RED, RGBA_WHITE } from "@/constants";
...
export const g_TextureListData = new SafetyMap<number, ITextureListData>(() => {
  return {
    ...
    search: "",
    rowTid: Array.from({ length: MAX_TEXTURELIST_ROWS }, () => INVALID_MODEL_ID),
...
function onDialogResponse(player: Player, res: IDialogResCommon, dialogId: number) {
  ...
  switch (dialogId) {
    case DIALOG_ID.TEXTURELIST_PAGE: {
      ...
      if (page < MIN_TEXTURELIST_PAGE || page > g_TextureListData.get(player.id).maxPage) {
        player.sendClientMessage(RGBA_RED, "ERROR: You did not enter a valid page number!");
        showTextureListDialog(player, dialogId);
        return 1;
      }
...
TextDrawEvent.onPlayerClickGlobal(({ player, textDraw, next }) => {
  ...
  if (textDraw === g_TextureListGTD.pageN) {
    if (g_TextureListData.get(player.id).page == g_TextureListData.get(player.id).maxPage) {
      return 1;
    }

    if (++g_TextureListData.get(player.id).page > g_TextureListData.get(player.id).maxPage) {
      g_TextureListData.get(player.id).page = g_TextureListData.get(player.id).maxPage;
    }
...
TextDrawEvent.onPlayerClickPlayer(({ player, textDraw, next }) => {
  ...
  for (let row = 0; row < MAX_TEXTURELIST_ROWS; row++) {
    if (
      ...
      g_ObjectData.get(objectId - 1).matIndexTexture[materialIndex] = rowTextureId;
...
export function showTextureList(player: Player) {
  ...
  Object.values(g_TextureListGTD).forEach((gtd: ITextureListGtd[keyof ITextureListGtd]) => {
    if (gtd && gtd.isValid()) {
      gtd.show(player);
    }
  });
...
export function getTextureListNewColoredRow(player: Player) {
  ...
  const materialIndex = g_PlayerData.get(player.id).editObjMatIdx;
  if (g_ObjectData.get(objectId - 1).matIndexType[materialIndex] != MATERIAL_INDEX_TYPE.TEXTURE) {
    return INVALID_ROW;
  }

  const objTextureId = g_ObjectData.get(objectId - 1).matIndexTexture[materialIndex];
...
export function applyTextureListCaption(player: Player) {
  const g_TextDrawString = `Texture Index ${g_PlayerData.get(player.id).editObjMatIdx}`;
...
export function applyTextureListRowColor(player: Player, row: number) {
  ...
  g_TextureListPTD.get(player.id).rowMid[row]?.setBoxColors(rgbaBoxColor).show();
  g_TextureListPTD.get(player.id).rowTxd[row]?.setBoxColors(rgbaBoxColor).show();
  g_TextureListPTD.get(player.id).rowName[row]?.setBoxColors(rgbaBoxColor).show();
  return 1;
}
...
export function applyTextureListRowData(player: Player) {
  for (let row = 0; row < MAX_TEXTURELIST_ROWS; row++) {
    ...
    g_TextureListPTD.get(player.id).rowName[row]?.setString(g_TextureNameString);
...
export function loadTextureListRowData(player: Player) {
  ...
  g_TextureListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_TEXTURELIST_ROWS);

  g_TextureListData.get(player.id).coloredRow = getTextureListNewColoredRow(player);
