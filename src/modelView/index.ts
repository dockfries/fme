...
import { GameMode, Player, PlayerEvent, TextDraw, TextDrawEvent } from "@infernus/core";
...
GameMode.onInit(({ next }) => {
  ...
  g_ModelViewTimer = setInterval(onModelViewUpdate, MODELVIEW_UPDATE_INTERVAL_MS);
  return next();
});
...
function onModelViewUpdate() {
  Player.getInstances().forEach((p) => {
    ...
    g_ModelViewData.get(p.id).rx = fixRot(g_ModelViewData.get(p.id).rx + speedRx);
    g_ModelViewData.get(p.id).ry = fixRot(g_ModelViewData.get(p.id).ry + speedRy);
    g_ModelViewData.get(p.id).rz = fixRot(g_ModelViewData.get(p.id).rz + speedRz);
...
export function destroyPlayerModelView(player: Player) {
  Object.entries(g_ModelViewPTD.get(player.id)).forEach((item) => {
    const [key, ptd] = item as unknown as [keyof IModelViewPtd, IModelViewPtd[keyof IModelViewPtd]];
...
export function showModelView(player: Player) {
  ...
  Object.values(g_ModelViewGTD).forEach((gtd: IModelViewGtd[keyof IModelViewGtd]) => {
    if (Array.isArray(gtd)) {
      gtd.forEach((_gtd) => {
        if (_gtd && _gtd.isValid()) {
          _gtd.show(player);
...
export function applyModelViewModel(player: Player, showTd: boolean) {
  g_ModelViewPTD.get(player.id).model?.setPreviewModel(g_ModelViewData.get(player.id).modelId);
