...
import { defaultCategorySelectData, loadCategorySelectData, showCategorySelect } from "@/catSelect";
import { INVALID_MODEL_ID, INVALID_ROW, RGBA_GREEN, RGBA_RED, RGBA_WHITE } from "@/constants";
import { DIALOG_ID } from "@/dialog";
import { getModelName, findModels } from "@/model";
import { g_ModelViewData, applyModelViewModel, showModelView, hideModelView } from "@/modelView";
import { g_PlayerData } from "@/player";
import { findSkins, getSkinName } from "@/skin";
import { hidePlayerTextDrawMode, showPlayerTextDrawMode, TD_MODE } from "@/tdMode";
...
export const g_CatManagerData = new SafetyMap<number, ICatManagerData>(() => {
  return {
    ...
    searchAll: "",
    searchCat: "",
    rowIdAll: Array.from({ length: MAX_CAT_MANAGER_ROWS }, () => INVALID_MODEL_ID),
    rowIdCat: Array.from({ length: MAX_CAT_MANAGER_ROWS }, () => INVALID_MODEL_ID),
  };
});
  ...
  switch (dialogId) {
    case DIALOG_ID.CATMANAGER_CAT_CREATE: {
      ...
      if (!inputText.trim().length) {
        ...
        player.sendClientMessage(RGBA_RED, "ERROR: You did not enter anything into the textfield!");
        return 1;
      }
      ...
      if (createSuccess) {
        ...
        player.sendClientMessage(RGBA_RED, "ERROR: Category could not be created!");
        showCategoryManagerDialog(player, dialogId);
      }
      return 1;
    }
    case DIALOG_ID.CATMANAGER_CAT_DESTROY: {
      ...
      if (inputText.toLocaleLowerCase() !== CAT_MANAGER_DELETE_CODE) {
        player.sendClientMessage(RGBA_RED, "ERROR: You did not enter the correct delete code!");
        showCategoryManagerDialog(player, dialogId);
        return 1;
      }
      ...
      if (success) {
        player.sendClientMessage(RGBA_GREEN, "Category destroyed successfully.");

        setCategoryManagerPage(player, CAT_MANAGER_WINDOW_CAT, MIN_CAT_MANAGER_PAGE);
        ...
        player.sendClientMessage(RGBA_RED, "ERROR: This category could not be removed!");
        showCategoryManagerDialog(player, dialogId);
      }
      return 1;
    }
    case DIALOG_ID.CATMANAGER_CAT_RENAME: {
      ...
      if (success) {
        ...
        player.sendClientMessage(RGBA_RED, "ERROR: This category could not be renamed!");
        showCategoryManagerDialog(player, dialogId);
      }
      return 1;
    }
    ...
    case DIALOG_ID.CATMANAGER_PAGE_CAT: {
      ...
      if (
        ...
        player.sendClientMessage(RGBA_RED, "ERROR: You did not enter a valid page number!");
        showCategoryManagerDialog(player, dialogId);
        return 1;
      }
...
TextDrawEvent.onPlayerClickGlobal(({ player, textDraw, next }) => {
  ...
  if (textDraw === g_CatManagerGTD.pageFAll || textDraw === g_CatManagerGTD.pageFCat) {
    const window =
      textDraw === g_CatManagerGTD.pageFAll ? CAT_MANAGER_WINDOW_ALL : CAT_MANAGER_WINDOW_CAT;
  ...
  if (textDraw === g_CatManagerGTD.pagePAll || textDraw === g_CatManagerGTD.pagePCat) {
    const window =
      textDraw === g_CatManagerGTD.pagePAll ? CAT_MANAGER_WINDOW_ALL : CAT_MANAGER_WINDOW_CAT;
  ...
  if (textDraw === g_CatManagerGTD.pageNAll || textDraw === g_CatManagerGTD.pageNCat) {
    const window =
      textDraw === g_CatManagerGTD.pageNAll ? CAT_MANAGER_WINDOW_ALL : CAT_MANAGER_WINDOW_CAT;
    let page = getCategoryManagerPage(player, window);
    const maxPage = getCategoryManagerMaxPage(player, window);
  ...
  if (textDraw === g_CatManagerGTD.pageLAll || textDraw === g_CatManagerGTD.pageLCat) {
    const window =
      textDraw === g_CatManagerGTD.pageLAll ? CAT_MANAGER_WINDOW_ALL : CAT_MANAGER_WINDOW_CAT;
    const maxPage = getCategoryManagerMaxPage(player, window);
...
export function showCategoryManager(player: Player) {
  ...
  Object.values(g_CatManagerGTD).forEach((gtd: ICatManagerGtd[keyof ICatManagerGtd]) => {
    if (gtd && gtd.isValid()) {
      gtd.show(player);
    }
  });
...
export function applyCategoryManagerRowColor(player: Player, window: boolean, row: number) {
  const selectWindow = g_CatManagerData.get(player.id).selectWindow;
  const selectRow = g_CatManagerData.get(player.id).selectRow;
  const rgbaBoxColor = selectWindow === window && row === selectRow ? 0xffffff64 : 0x00000000;

  if (window === CAT_MANAGER_WINDOW_ALL) {
    g_CatManagerPTD.get(player.id).row1All[row]?.setBoxColors(rgbaBoxColor)?.show();
    g_CatManagerPTD.get(player.id).row2All[row]?.setBoxColors(rgbaBoxColor)?.show();

    if (g_CatManagerPTD.get(player.id).row3All[row]) {
      g_CatManagerPTD.get(player.id).row3All[row]?.setBoxColors(rgbaBoxColor)?.show();
    }
  } else if (window === CAT_MANAGER_WINDOW_CAT) {
    g_CatManagerPTD.get(player.id).row1Cat[row]?.setBoxColors(rgbaBoxColor)?.show();
    g_CatManagerPTD.get(player.id).row2Cat[row]?.setBoxColors(rgbaBoxColor)?.show();

    if (g_CatManagerPTD.get(player.id).row3Cat[row]) {
      g_CatManagerPTD.get(player.id).row3Cat[row]?.setBoxColors(rgbaBoxColor)?.show();
    }
  }
}
  ...
  for (let row = 0; row < MAX_CAT_MANAGER_ROWS; row++) {
    ...
    switch (g_PlayerData.get(player.id).tdMode) {
      case TD_MODE.CATMANAGER_MODELS: {
        ...
        if (window === CAT_MANAGER_WINDOW_ALL) {
          g_CatManagerPTD.get(player.id).row1All[row]?.setString(g_IntegerString);
          g_CatManagerPTD.get(player.id).row2All[row]?.setString(g_ModelString);
        } else {
          g_CatManagerPTD.get(player.id).row1Cat[row]?.setString(g_IntegerString);
      ...
      case TD_MODE.CATMANAGER_VEHICLES: {
        ...
        if (window === CAT_MANAGER_WINDOW_ALL) {
          g_CatManagerPTD.get(player.id).row1All[row]?.setString(g_IntegerString);
          g_CatManagerPTD.get(player.id).row2All[row]?.setString(g_VehModelString);
        } else {
          g_CatManagerPTD.get(player.id).row1Cat[row]?.setString(g_IntegerString);
          g_CatManagerPTD.get(player.id).row2Cat[row]?.setString(g_VehModelString);
        }
        break;
      }
      case TD_MODE.CATMANAGER_SKINS: {
      ...
      case TD_MODE.CATMANAGER_TEXTURES: {
        ...
        if (window === CAT_MANAGER_WINDOW_ALL) {
          g_CatManagerPTD.get(player.id).row1All[row]?.setString(g_IntegerString);
          g_CatManagerPTD.get(player.id).row2All[row]?.setString(g_TextureTXDString);
          g_CatManagerPTD.get(player.id).row3All[row]?.setString(g_TextureNameString);
        } else {
          g_CatManagerPTD.get(player.id).row1Cat[row]?.setString(g_IntegerString);
          g_CatManagerPTD.get(player.id).row2Cat[row]?.setString(g_TextureTXDString);
          g_CatManagerPTD.get(player.id).row3Cat[row]?.setString(g_TextureNameString);
        }
        break;
      }
    }
...
export function loadCategoryManagerRowData(player: Player, window: boolean) {
  ...
  if (window === CAT_MANAGER_WINDOW_ALL) {
    g_CatManagerData.get(player.id).maxPageAll = Math.floor(maxOffset / MAX_CAT_MANAGER_ROWS);
  } else {
    g_CatManagerData.get(player.id).maxPageCat = Math.floor(maxOffset / MAX_CAT_MANAGER_ROWS);
  }

  g_CatManagerData.get(player.id).selectRow = getCategoryManagerNewSelectRow(player);
  return 1;
}
...
export async function showCategoryManagerDialog(player: Player, dialogId: number) {
  switch (dialogId) {
    case DIALOG_ID.CATMANAGER_CAT_CREATE: {
...
export function setCategoryManagerPage(player: Player, window: boolean, page: number) {
  switch (window) {
    case CAT_MANAGER_WINDOW_ALL: {
...
export function setCategoryManagerSearch(player: Player, window: boolean, search: string) {
  switch (window) {
    case CAT_MANAGER_WINDOW_ALL: {
...
export function getCategoryManagerRowID(player: Player, window: boolean, row: number) {
  let id = 0;
  switch (window) {
    case CAT_MANAGER_WINDOW_ALL: {
...
export function setCategoryManagerRowID(player: Player, window: boolean, row: number, id: number) {
  switch (window) {
    case CAT_MANAGER_WINDOW_ALL: {
