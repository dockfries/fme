...
import { applySelectListRowData, g_SelectVehListData, MAX_SELECT_LIST_ROWS } from "@/selectList";
...
import { g_VehModelNameCache, getVehicleModelCacheIndex, getVehicleModelName } from "@/vehModel";
import {
  Dialog,
  DialogStylesEnum,
      ...
      switch (listItem) {
        ...
        case DIALOG_LISTITEM_VEHICLE.DRIVE: {
          if (isVehicleOccupied(vehicleId, 0)) {
            player.sendClientMessage(RGBA_RED, "ERROR: Someone is already driving this vehicle!");
          } else if (player.getState() === PlayerStateEnum.SPECTATING) {
            player.sendClientMessage(RGBA_RED, "ERROR: You need to spawned to do this!");
          } else {
        ...
        case DIALOG_LISTITEM_VEHICLE.MOVE: {
          ...
          if (objectId === InvalidEnum.OBJECT_ID) {
            player.sendClientMessage(RGBA_RED, "ERROR: This vehicle cannot be moved right now!");
          } else {
            const { x, y, z } = veh.getPos();
        ...
        case DIALOG_LISTITEM_VEHICLE.DUPLICATE: {
          ...
          if (newVehicleId === InvalidEnum.VEHICLE_ID) {
            player.sendClientMessage(RGBA_RED, "ERROR: This vehicle could not be duplicated!");
          } else {
        ...
        case DIALOG_LISTITEM_VEHICLE.RECREATE: {
          ...
          if (newVehicleId === InvalidEnum.VEHICLE_ID) {
            player.sendClientMessage(RGBA_RED, "ERROR: This vehicle could not be re-created!");
          }
          break;
        }
        ...
        case DIALOG_LISTITEM_VEHICLE.MODSHOP: {
          ...
          if (player.getVehicle()?.id !== vehicleId || player.getVehicleSeat() !== 0) {
            if (isVehicleOccupied(vehicleId, 0)) {
              player.sendClientMessage(RGBA_RED, "ERROR: Someone else is driving this vehicle!");
              showVehicleDialog(player, dialogId);
              return 1;
            } else {
              veh.putPlayerIn(player, 0);
...
export async function showVehicleDialog(player: Player, dialogId: number) {
  switch (dialogId) {
    case DIALOG_ID.VEHICLE_MAIN: {
      ...
      for (let listItem = 0; listItem < DIALOG_LISTITEM_VEHICLE.MAX; listItem++) {
        switch (listItem) {
          case DIALOG_LISTITEM_VEHICLE.GOTO: {
            g_DialogInfo += "Goto\t \n";
            break;
          ...
          case DIALOG_LISTITEM_VEHICLE.COLOR1: {
            ...
            if (colorId === INVALID_COLOR_ID) {
              ...
              const rgb = getVehicleColorRGB(colorId).toString(16).padStart(6, "0");
...
export function copyVehicle(copyVehicleId: number, a: number) {
  ...
  try {
    ...
    g_VehicleData.get(pasteVehicleId - 1).memoryX = g_VehicleData.get(copyVehicleId - 1).memoryX;
    g_VehicleData.get(pasteVehicleId - 1).memoryY = g_VehicleData.get(copyVehicleId - 1).memoryY;
    g_VehicleData.get(pasteVehicleId - 1).memoryZ = g_VehicleData.get(copyVehicleId - 1).memoryZ;
    g_VehicleData.get(pasteVehicleId - 1).memoryRz = g_VehicleData.get(copyVehicleId - 1).memoryRz;
...
export function copyVehicleAttachments(fromVehicleId: number, toVehicleId: number) {
...
export function transferVehicleAttachments(fromVehicleId: number, toVehicleId: number) {
  if (!Vehicle.isValid(fromVehicleId) || !Vehicle.isValid(toVehicleId)) {
    return 0;
  }
  ObjectMp.getInstances().forEach((o) => {
...
export function isVehicleOccupied(vehicleId: number, seat: number) {
  return Player.getInstances().some(
    (p) => p.isConnected() && p.getVehicle()?.id === vehicleId && p.getVehicleSeat() === seat,
  );
}

export function findVehicles(result: number[], resultSize: number, search: string, offset: number) {
  let rowsFound = 0;
  let rowsAdded = 0;
  let maxOffset = 0;
  ...
  Vehicle.getInstances().forEach((v) => {
    ...
    if (
      ...
      (cacheIdx !== INVALID_ARRAY_INDEX && g_VehModelNameCache[cacheIdx].includes(search))
    ) {
      if (rowsFound++ < offset) {
        return;
      }
