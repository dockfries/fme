...
import { GameMode, InvalidEnum, Player, TextDraw, TextDrawEvent } from "@infernus/core";
...
export const g_HelpGTD: IHelpGtd = {
  ...
  creditTab: null,
  creditCaption: null,
  creditL: Array.from({ length: HELP_CREDIT.MAX }, () => null),
  creditC: Array.from({ length: HELP_CREDIT.MAX }, () => null),
  creditR: Array.from({ length: HELP_CREDIT.MAX }, () => null),
};
...
export function createGenericHelpWindow() {
  ...
  g_TextDrawString += "This map editor has every tool you need to make a high quality map.";
  ...
  for (let row = 0, y = 80.0; row < HELP_KEYBIND.MAX; row++, y += 12.0) {
    ...
    switch (row) {
      ...
      case HELP_KEYBIND.OFFEDIT_MODE: {
        g_HelpGTD.keyBindL[row]?.setString("Hold ~k~~PED_SPRINT~ / ~k~~VEHICLE_ACCELERATE~");
      ...
      case HELP_KEYBIND.MOUSE_ON: {
        g_HelpGTD.keyBindL[row]?.setString("~k~~CONVERSATION_YES~ / ~k~~PED_DUCK~");
...
export function destroyGenericHelpWindow() {
  Object.entries(g_HelpGTD).forEach((item) => {
    const [key, gtd] = item as unknown as [keyof IHelpGtd, (typeof g_HelpGTD)[keyof IHelpGtd]];
