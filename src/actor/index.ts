...
import { applySelectListRowData, g_SelectActListData, MAX_SELECT_LIST_ROWS } from "@/selectList";
    ...
    if (
      ...
      switch (response) {
        ...
        case EditResponseTypesEnum.CANCEL: {
          ...
          const newActor = recreateActor(actorId, g_ActorData.get(actor.id).memoryA);
          if (newActor === InvalidEnum.ACTOR_ID) {
            player.sendClientMessage(
              RGBA_RED,
              "ERROR: This actor could not be rotated / re-created!",
  ...
  switch (dialogId) {
      ...
      switch (listItem) {
        case DIALOG_LISTITEM_ACTOR.GOTO: {
          ...
          if (g_CamModeData.get(player.id).toggle) {
            const playerObject = ObjectMp.getInstance(g_CamModeData.get(player.id).poId, player);
            if (playerObject) {
              playerObject.stop();
        ...
        case DIALOG_LISTITEM_ACTOR.MOVE: {
          ...
          if (!object) {
            player.sendClientMessage(RGBA_RED, "ERROR: This actor cannot be moved right now!");
          } else {
            const { x, y, z } = actor.getPos();
        ...
        case DIALOG_LISTITEM_ACTOR.DUPLICATE: {
          ...
          if (newActorId === InvalidEnum.ACTOR_ID) {
            player.sendClientMessage(RGBA_RED, "ERROR: This actor could not be duplicated!");
          } else {
        ...
        case DIALOG_LISTITEM_ACTOR.ANIM_LOOP: {
          g_ActorData.get(actor.id).animLoop = !g_ActorData.get(actor.id).animLoop;
          applyActorAnimationData(actor);
          break;
        }
        case DIALOG_LISTITEM_ACTOR.ANIM_LOCKX: {
          g_ActorData.get(actor.id).animLockX = !g_ActorData.get(actor.id).animLockX;
          applyActorAnimationData(actor);
          break;
        }
        case DIALOG_LISTITEM_ACTOR.ANIM_LOCKY: {
          g_ActorData.get(actor.id).animLockY = !g_ActorData.get(actor.id).animLockY;
          applyActorAnimationData(actor);
          break;
        }
        case DIALOG_LISTITEM_ACTOR.ANIM_FREEZE: {
          g_ActorData.get(actor.id).animFreeze = !g_ActorData.get(actor.id).animFreeze;
          applyActorAnimationData(actor);
          break;
        }
    ...
    case DIALOG_ID.ACTOR_ANIM_TIME: {
      ...
      if (!inputText.trim().length || Number.isNaN(time)) {
        player.sendClientMessage(RGBA_RED, "ERROR: You did not enter a numeric value!");
      } else if (time < 0) {
        player.sendClientMessage(RGBA_RED, "ERROR: You cannot enter a negative value!");
      } else {
        ...
        switch (listItem) {
          ...
          case DIALOG_LISTITEM_ACTOR.ANIM_DELTA: {
            if (g_ActorData.get(actor.id).animIndex === INVALID_ANIM_INDEX) {
              ...
              const g_DialogInfoRow = `Animation Delta\t${g_ActorData.get(actor.id).animDelta}\n`;
              g_DialogInfo += g_DialogInfoRow;
            }
            break;
          }
          ...
          case DIALOG_LISTITEM_ACTOR.ANIM_TIME: {
            if (g_ActorData.get(actor.id).animIndex === INVALID_ANIM_INDEX) {
              ...
              const g_DialogInfoRow = `Animation Time\t${g_ActorData.get(actor.id).animTime}\n`;
              g_DialogInfo += g_DialogInfoRow;
            }
            break;
          }
...
export function findActors(result: number[], resultSize: number, search: string, offset: number) {
  let rowsFound = 0;
  let rowsAdded = 0;
  let searchInt = -1;
