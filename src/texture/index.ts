...
export function loadTextureCache() {
  const g_DBResult = g_ResourceDB.prepare("SELECT * FROM texture_data").all() as {
    textureid: number;
    modelid: number;
    txd: string;
    name: string;
...
export function getTextureID(modelId: number, txd: string, name: string) {
  ...
  const g_DBResult = g_ResourceDB.prepare(g_QueryString).get(modelId, txd, name) as
    | { textureid: number }
    | undefined;
...
export function findTextures(
  ...
  if (categoryId === INVALID_CATEGORY_ID) {
    ...
    if (!search.trim().length) {
      ...
      g_QueryString =
        "\
                SELECT b.textureid AS textureid, \
                (\
