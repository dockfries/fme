...
export function getMapID(mapName: string) {
  ...
  const g_DBResult = g_MapDB.prepare(g_QueryString).get(mapName.toLowerCase()) as
    | { mapid: number }
    | undefined;
  if (g_DBResult) {
...
export function getMapName(mapId: number) {
  ...
  const g_DBResult = g_MapDB.prepare(g_QueryString).get(mapId) as { name: string } | undefined;
...
export function findMaps(result: number[], resultSize: number, search: string, offset: number) {
  let rowsAdded = 0;
  let maxOffset = 0;
  ...
  if (!search.trim().length) {
    g_QueryString =
      "\
            ...
            FROM maps \
            LIMIT ? OFFSET ?";
    g_DBResult = g_MapDB.prepare(g_QueryString).all(resultSize, offset) as typeof g_DBResult;
  } else {
