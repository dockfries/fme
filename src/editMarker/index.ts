...
import { g_BuildingData, INVALID_BUILDING_ID, isValidBuildingID } from "@/building";
import { INVALID_MODEL_ID, RGBA_YELLOW } from "@/constants";
import { ID_TYPE } from "@/idType";
import { getModelMaxZ } from "@/model";
import { getObjectAttachObject, getObjectAttachVehicle, g_ObjectData } from "@/object";
...
export function refreshPlayerEditMarker(player: Player) {
  ...
  if (unToggle) {
    if (g_EditMarkerData.get(player.id).on) {
      ...
      if (ObjectMp.isValid(g_EditMarkerData.get(player.id).poId, player.id)) {
        ObjectMp.getInstance(g_EditMarkerData.get(player.id).poId, player)!.destroy();
      }
  ...
  if (g_EditMarkerData.get(player.id).attach && !Vehicle.isValid(attachToVehicleId)) {
    if (ObjectMp.isValid(g_EditMarkerData.get(player.id).poId, player.id)) {
      ObjectMp.getInstance(g_EditMarkerData.get(player.id).poId, player)!.destroy();
    }
  ...
  if (ObjectMp.isValid(g_EditMarkerData.get(player.id).poId, player.id)) {
    if (Vehicle.isValid(attachToVehicleId)) {
      const pObj = ObjectMp.getInstance(g_EditMarkerData.get(player.id).poId, player);
      if (pObj) {
  ...
  player.setMapIcon(EDITMARKER_MAPICONID, x, y, z, 0, RGBA_YELLOW, MapIconStyles.GLOBAL);
  return 1;
}
