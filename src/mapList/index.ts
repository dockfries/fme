...
export function loadMapListData(player: Player) {
  ...
  g_MapListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_MAPLIST_ROWS);
}
...
async function onDialogResponse(player: Player, res: IDialogResCommon, dialogId: number) {
  ...
  switch (dialogId) {
      ...
      if (listItem >= MAP_LIST_ITEM.ROW_F && listItem <= MAP_LIST_ITEM.ROW_L) {
        ...
        switch (listItem) {
          ...
          case MAP_LIST_ITEM.PAGE_N: {
            if (++g_MapListData.get(player.id).page > g_MapListData.get(player.id).maxPage) {
              g_MapListData.get(player.id).page = g_MapListData.get(player.id).maxPage;
            }
            loadMapListData(player);
            break;
          }
    ...
    case DIALOG_ID.MAPLIST_PAGE: {
      ...
      if (page < MIN_MAPLIST_PAGE || page > g_MapListData.get(player.id).maxPage) {
        player.sendClientMessage(RGBA_RED, "ERROR: You did not enter a valid page!");
        showMapListDialog(player, dialogId);
        return 1;
      }
...
export async function showMapListDialog(player: Player, dialogId: number) {
  switch (dialogId) {
    ...
    case DIALOG_ID.MAPLIST_PAGE: {
      ...
      const g_DialogInfo = `Current Page: ${g_MapListData.get(player.id).page + 1}`;
      const res = await new Dialog({
