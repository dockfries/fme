...
export function toggleCam(player: Player, toggle: boolean) {
  ...
  if (toggle) {
    ...
    player.sendClientMessage(RGBA_ORANGE, "Faster: {FFFFFF}~k~~PED_JUMPING~ + Direction Key");
    player.sendClientMessage(RGBA_ORANGE, "Slower: {FFFFFF}~k~~SNEAK_ABOUT~ + Direction Key");
