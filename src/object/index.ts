...
import { DIALOG_ID, DIALOG_LISTITEM_OBJECT, DIALOG_LISTITEM_OINDEX } from "@/dialog";
...
import { applySelectListRowData, g_SelectObjListData, MAX_SELECT_LIST_ROWS } from "@/selectList";
...
export const g_ObjectData = new SafetyMap<number, IObjectData>(() => {
  return {
    ...
    matIndexTexture: Array.from({ length: MAX_MATERIALINDEX_MODCOUNT }, () => INVALID_TEXTURE_ID),
    matIndexSize: Array.from({ length: MAX_MATERIALINDEX_MODCOUNT }, () => 0),
    matIndexFontSize: Array.from({ length: MAX_MATERIALINDEX_MODCOUNT }, () => 24),
    matIndexIsBold: Array.from({ length: MAX_MATERIALINDEX_MODCOUNT }, () => true),
    matIndexColor: Array.from({ length: MAX_MATERIALINDEX_MODCOUNT }, () => 0),
    matIndexFontColor: Array.from({ length: MAX_MATERIALINDEX_MODCOUNT }, () => 0xffffffff),
    matIndexAlignment: Array.from({ length: MAX_MATERIALINDEX_MODCOUNT }, () => 0),
    comment: "",
  };
});
...
GameMode.onInit(({ next }) => {
  ObjectMp.getInstances().forEach((o) => {
    ...
    for (let materialIndex = 0; materialIndex < MAX_OBJECT_INDEX; materialIndex++) {
      g_ObjectData.get(o.id - 1).matIndexType[materialIndex] = MATERIAL_INDEX_TYPE.NONE;
    }
  });
  return next();
});
...
ObjectMpEvent.onPlayerEdit(
  ({ player, isPlayerObject, objectMp, response, fX, fY, fZ, fRotX, fRotY, fRotZ, next }) => {
...
function onDialogResponse(player: Player, res: IDialogResCommon, dialogId: number) {
  ...
  switch (dialogId) {
    case DIALOG_ID.OBJECT_MAIN: {
      ...
      if (
        ...
        g_PlayerData.get(player.id).editObjMatIdx = listItem - DIALOG_LISTITEM_OBJECT.INDEX_START;
        ...
        switch (listItem) {
          case DIALOG_LISTITEM_OBJECT.GOTO: {
            ...
            if (g_CamModeData.get(player.id).toggle) {
              const pObj = ObjectMp.getInstance(g_CamModeData.get(player.id).poId, player)!;
              pObj?.stop();
              pObj?.setPos(x, y, z);
            } else {
              player.setPos(x, y, z);
          ...
          case DIALOG_LISTITEM_OBJECT.ATTACH_APPLY: {
            ...
            if (ObjectMp.isValid(attachObjectId) && attachObjectId !== objectId) {
              g_ObjectData.get(attachObjectId - 1).attachIdType = ID_TYPE.OBJECT;
          ...
          case DIALOG_LISTITEM_OBJECT.MATERIALS_PASTE: {
            const copyFromObjectId = g_PlayerData.get(player.id).editMaterialObj;
            if (ObjectMp.isValid(copyFromObjectId) && copyFromObjectId !== objectId) {
              for (let matIdx = 0; matIdx < MAX_OBJECT_INDEX; matIdx++) {
                defaultObjectMaterialIndexData(objectId, matIdx);
                migrateObjectMaterialIndexData(copyFromObjectId, objectId, matIdx);
              }
          ...
          case DIALOG_LISTITEM_OBJECT.DUPLICATE: {
            ...
            if (newObjectId === InvalidEnum.OBJECT_ID) {
              player.sendClientMessage(RGBA_RED, "ERROR: This object could not be duplicated!");
            } else {
          ...
          case DIALOG_LISTITEM_OBJECT.RECREATE: {
            const newObjectId = recreateObject(objectId);
            if (newObjectId === InvalidEnum.OBJECT_ID) {
              player.sendClientMessage(RGBA_RED, "ERROR: This object could not be re-created!");
            }
            break;
          }
    ...
    case DIALOG_ID.OBJECT_INDEX: {
      ...
      switch (listItem) {
        case DIALOG_LISTITEM_OINDEX.REMOVE: {
          g_ObjectData.get(objectId - 1).matIndexType[g_PlayerData.get(player.id).editObjMatIdx] =
            MATERIAL_INDEX_TYPE.NONE;
        ...
        case DIALOG_LISTITEM_OINDEX.MAT_COLOR_R: {
          ...
          if (g_ObjectData.get(objectId - 1).matIndexModCount >= MAX_MATERIALINDEX_MODCOUNT) {
            const newObjectId = recreateObject(objectId);
            if (newObjectId === InvalidEnum.OBJECT_ID) {
              player.sendClientMessage(
                RGBA_RED,
        ...
        case DIALOG_LISTITEM_OINDEX.TXT_ISBOLD: {
          const materialIndex = g_PlayerData.get(player.id).editObjMatIdx;
          if (
            g_ObjectData.get(objectId - 1).matIndexType[materialIndex] != MATERIAL_INDEX_TYPE.TEXT
          ) {
            g_ObjectData.get(objectId - 1).matIndexType[materialIndex] = MATERIAL_INDEX_TYPE.TEXT;
            defaultObjectMaterialIndexData(objectId, materialIndex);
          }

          g_ObjectData.get(objectId - 1).matIndexIsBold[materialIndex] = !g_ObjectData.get(
            objectId - 1,
          ).matIndexIsBold[materialIndex];
        ...
        case DIALOG_LISTITEM_OINDEX.TXT_BACKCOLOR_O: {
          ...
          g_ObjectData.get(objectId - 1).matIndexColor[materialIndex] = setARGBAlpha(
    ...
    case DIALOG_ID.OBJECT_MATSIZE: {
      ...
      const materialIndex = g_PlayerData.get(player.id).editObjMatIdx;
      if (g_ObjectData.get(objectId - 1).matIndexType[materialIndex] != MATERIAL_INDEX_TYPE.TEXT) {
        g_ObjectData.get(objectId - 1).matIndexType[materialIndex] = MATERIAL_INDEX_TYPE.TEXT;
        defaultObjectMaterialIndexData(objectId, materialIndex);
      }

      g_ObjectData.get(objectId - 1).matIndexSize[materialIndex] = (listItem + 1) * 10;
    ...
    case DIALOG_ID.COLORALPHA_FONTFACE: {
      ...
      if (!inputText.trim().length || Number.isNaN(alpha)) {
        player.sendClientMessage(RGBA_RED, "ERROR: You must enter a decimal or hexadecimal value!");
        showObjectDialog(player, dialogId);
        return 1;
      }

      if (alpha < 0x00 || alpha > 0xff) {
        player.sendClientMessage(RGBA_RED, "ERROR: You must enter a value between 0 - 255!");
        showObjectDialog(player, dialogId);
        return 1;
      }
        ...
        case DIALOG_ID.COLORALPHA_FONTFACE:
          {
            ...
            g_ObjectData.get(objectId - 1).matIndexFontColor[materialIndex] = setARGBAlpha(
...
export function migrateObjectMaterialIndexData(
  ...
  switch (g_ObjectData.get(fromObjectId - 1).matIndexType[materialIndex]) {
    case MATERIAL_INDEX_TYPE.TEXTURE: {
      g_ObjectData.get(toObjectId - 1).matIndexTexture[materialIndex] = g_ObjectData.get(
        fromObjectId - 1,
      ).matIndexTexture[materialIndex];
      g_ObjectData.get(toObjectId - 1).matIndexColor[materialIndex] = g_ObjectData.get(
        fromObjectId - 1,
      ).matIndexColor[materialIndex];
      break;
    }
    case MATERIAL_INDEX_TYPE.TEXT: {
      const g_ObjectTextString = g_ObjectText.get(fromObjectId - 1)[materialIndex];
      ...
      g_ObjectData.get(toObjectId - 1).matIndexSize[materialIndex] = g_ObjectData.get(
        fromObjectId - 1,
      ).matIndexSize[materialIndex];
      g_ObjectData.get(toObjectId - 1).matIndexFontSize[materialIndex] = g_ObjectData.get(
        fromObjectId - 1,
      ).matIndexFontSize[materialIndex];
      g_ObjectData.get(toObjectId - 1).matIndexIsBold[materialIndex] = g_ObjectData.get(
        fromObjectId - 1,
      ).matIndexIsBold[materialIndex];
      g_ObjectData.get(toObjectId - 1).matIndexFontColor[materialIndex] = g_ObjectData.get(
        fromObjectId - 1,
      ).matIndexFontColor[materialIndex];
      g_ObjectData.get(toObjectId - 1).matIndexColor[materialIndex] = g_ObjectData.get(
        fromObjectId - 1,
      ).matIndexColor[materialIndex];
      g_ObjectData.get(toObjectId - 1).matIndexAlignment[materialIndex] = g_ObjectData.get(
        fromObjectId - 1,
  ...
  g_ObjectData.get(toObjectId - 1).matIndexType[materialIndex] = g_ObjectData.get(
    fromObjectId - 1,
  ).matIndexType[materialIndex];
  return 1;
}
...
export function migrateObjectAttachData(
  ...
  g_ObjectData.get(toObjectId - 1).attachIdType = g_ObjectData.get(fromObjectId - 1).attachIdType;
  g_ObjectData.get(toObjectId - 1).attachId = attachToId;
  g_ObjectData.get(toObjectId - 1).attachX = g_ObjectData.get(fromObjectId - 1).attachX;
  g_ObjectData.get(toObjectId - 1).attachY = g_ObjectData.get(fromObjectId - 1).attachY;
  g_ObjectData.get(toObjectId - 1).attachZ = g_ObjectData.get(fromObjectId - 1).attachZ;
  g_ObjectData.get(toObjectId - 1).attachRx = g_ObjectData.get(fromObjectId - 1).attachRx;
  g_ObjectData.get(toObjectId - 1).attachRy = g_ObjectData.get(fromObjectId - 1).attachRy;
  g_ObjectData.get(toObjectId - 1).attachRz = g_ObjectData.get(fromObjectId - 1).attachRz;
  return 1;
}

export function defaultObjectMaterialIndexData(objectId: number, materialIndex: number) {
  switch (g_ObjectData.get(objectId - 1).matIndexType[materialIndex]) {
    case MATERIAL_INDEX_TYPE.TEXTURE: {
      g_ObjectData.get(objectId - 1).matIndexTexture[materialIndex] = INVALID_TEXTURE_ID;
      g_ObjectData.get(objectId - 1).matIndexColor[materialIndex] = 0x0;
      break;
    }
    case MATERIAL_INDEX_TYPE.TEXT: {
      ...
      g_ObjectData.get(objectId - 1).matIndexFontColor[materialIndex] = 0xffffffff;
      g_ObjectData.get(objectId - 1).matIndexColor[materialIndex] = 0x0;
      g_ObjectData.get(objectId - 1).matIndexAlignment[materialIndex] = 0;
      break;
    }
...
export function applyObjectMaterialIndexData(objectId: number, materialIndex: number) {
  switch (g_ObjectData.get(objectId - 1).matIndexType[materialIndex]) {
    case MATERIAL_INDEX_TYPE.TEXTURE: {
      const textureId = g_ObjectData.get(objectId - 1).matIndexTexture[materialIndex];
      const {
        modelId,
...
export function applyObjectAttachData(objectId: number) {
  switch (g_ObjectData.get(objectId - 1).attachIdType) {
    case ID_TYPE.OBJECT: {
      const obj = ObjectMp.getInstance(objectId);
      const attachObj = ObjectMp.getInstance(g_ObjectData.get(objectId - 1).attachId);
      if (obj && attachObj) {
        obj?.attachToObject(
          attachObj,
    ...
    case ID_TYPE.VEHICLE: {
      const obj = ObjectMp.getInstance(objectId);
      const attachVehicle = Vehicle.getInstance(g_ObjectData.get(objectId - 1).attachId);
...
export function copyObject(copyObjectId: number, copyAttachTo = true) {
  ...
  if (isValidAttachToObject || isValidAttachToVehicle) {
    ...
    const pos = positionFromOffset(attX, attY, attZ, attRx, attRy, attRz, offX, offY, offZ);
    x = pos.x;
    y = pos.y;
    z = pos.z;
  ...
  try {
    ...
    for (let materialIndex = 0; materialIndex < MAX_OBJECT_INDEX; materialIndex++) {
      if (migrateObjectMaterialIndexData(copyObjectId, pasteObjectId, materialIndex)) {
    ...
    g_ObjectData.get(pasteObjectId - 1).memoryX = g_ObjectData.get(copyObjectId - 1).memoryX;
    g_ObjectData.get(pasteObjectId - 1).memoryY = g_ObjectData.get(copyObjectId - 1).memoryY;
    g_ObjectData.get(pasteObjectId - 1).memoryZ = g_ObjectData.get(copyObjectId - 1).memoryZ;
    g_ObjectData.get(pasteObjectId - 1).memoryRx = g_ObjectData.get(copyObjectId - 1).memoryRx;
    g_ObjectData.get(pasteObjectId - 1).memoryRy = g_ObjectData.get(copyObjectId - 1).memoryRy;
    g_ObjectData.get(pasteObjectId - 1).memoryRz = g_ObjectData.get(copyObjectId - 1).memoryRz;
...
export function copyObjectAttachments(fromObjectId: number, toObjectId: number) {
  ObjectMp.getInstances().forEach((o) => {
...
export function transferObjectAttachments(fromObjectId: number, toObjectId: number) {
  ObjectMp.getInstances().forEach((o) => {
    if (!o.isValid() || o.id === fromObjectId || o.id === toObjectId) {
      return;
    }
...
export async function showObjectDialog(player: Player, dialogId: number) {
  switch (dialogId) {
    case DIALOG_ID.OBJECT_MAIN: {
      ...
      for (let listItem = 0; listItem < DIALOG_LISTITEM_OBJECT.MAX; listItem++) {
        if (
          listItem >= DIALOG_LISTITEM_OBJECT.INDEX_START &&
          listItem <= DIALOG_LISTITEM_OBJECT.INDEX_END
        ) {
          ...
          switch (g_ObjectData.get(objectId - 1).matIndexType[materialIndex]) {
            case MATERIAL_INDEX_TYPE.TEXTURE: {
              const textureId = g_ObjectData.get(objectId - 1).matIndexTexture[materialIndex];
              const colorArgb = g_ObjectData.get(objectId - 1).matIndexColor[materialIndex];
              const colorRgb = aRGBtoRGB(colorArgb).toString(16).padStart(6, "0");
            ...
            case MATERIAL_INDEX_TYPE.TEXT: {
              const colorArgb = g_ObjectData.get(objectId - 1).matIndexFontColor[materialIndex];
              const colorRgb = aRGBtoRGB(colorArgb).toString(16).padStart(6, "0");
          ...
          switch (listItem) {
            ...
            case DIALOG_LISTITEM_OBJECT.ATTACH_APPLY: {
              ...
              if (!ObjectMp.isValid(attachObject) || attachObject === objectId) {
                g_DialogInfo += " \t \n";
              } else {
                const g_CommentString = g_ObjectData.get(attachObject - 1).comment;
            ...
            case DIALOG_LISTITEM_OBJECT.ATTACH_REMOVE: {
              switch (g_ObjectData.get(objectId - 1).attachIdType) {
                ...
                case ID_TYPE.VEHICLE: {
                  ...
                  const g_CommentString = g_VehicleData.get(attachVehicle - 1).comment;
            ...
            case DIALOG_LISTITEM_OBJECT.MATERIALS_PASTE: {
              const copyFromObjectid = g_PlayerData.get(player.id).editMaterialObj;
              if (!ObjectMp.isValid(copyFromObjectid) || objectId === copyFromObjectid) {
                g_DialogInfo += " \t \n";
              } else {
                const g_CommentString = g_ObjectData.get(copyFromObjectid - 1).comment;
    ...
    case DIALOG_ID.OBJECT_INDEX: {
      ...
      const materialIndex = g_PlayerData.get(player.id).editObjMatIdx;
      const indexType = g_ObjectData.get(objectId - 1).matIndexType[materialIndex];

      const g_DialogCaption = `Material Index: ${materialIndex}`;
      let g_DialogInfo = "";

      for (let listItem = 0; listItem < DIALOG_LISTITEM_OINDEX.MAX; listItem++) {
        switch (listItem) {
          case DIALOG_LISTITEM_OINDEX.REMOVE: {
          ...
          case DIALOG_LISTITEM_OINDEX.MAT_COLOR_S: {
            if (indexType === MATERIAL_INDEX_TYPE.TEXTURE) {
              const argb = g_ObjectData.get(objectId - 1).matIndexColor[materialIndex];
          ...
          case DIALOG_LISTITEM_OINDEX.MAT_COLOR_A: {
            if (indexType === MATERIAL_INDEX_TYPE.TEXTURE) {
              const argb = g_ObjectData.get(objectId - 1).matIndexColor[materialIndex];
              const g_DialogInfoRow = `Texture\tAlpha\t${aRGBtoA(argb)}/${0xff}\n`;
              g_DialogInfo += g_DialogInfoRow;
            } else {
          ...
          case DIALOG_LISTITEM_OINDEX.TXT_MATSIZE: {
            if (indexType === MATERIAL_INDEX_TYPE.TEXT) {
              const materialSize = g_ObjectData.get(objectId - 1).matIndexSize[materialIndex];
          ...
          case DIALOG_LISTITEM_OINDEX.TXT_FONTSIZE: {
            if (indexType === MATERIAL_INDEX_TYPE.TEXT) {
              const fontSize = g_ObjectData.get(objectId - 1).matIndexFontSize[materialIndex];
          ...
          case DIALOG_LISTITEM_OINDEX.TXT_ISBOLD: {
            if (indexType === MATERIAL_INDEX_TYPE.TEXT) {
              const yesOrNo = g_ObjectData.get(objectId - 1).matIndexIsBold[materialIndex]
                ? "Yes"
                : "No";
          ...
          case DIALOG_LISTITEM_OINDEX.TXT_ALIGN: {
            if (indexType === MATERIAL_INDEX_TYPE.TEXT) {
              const alignment = g_ObjectData.get(objectId - 1).matIndexAlignment[materialIndex];
          ...
          case DIALOG_LISTITEM_OINDEX.TXT_FONTCOLOR_S: {
            if (indexType === MATERIAL_INDEX_TYPE.TEXT) {
              const fontColor = g_ObjectData.get(objectId - 1).matIndexFontColor[materialIndex];
          ...
          case DIALOG_LISTITEM_OINDEX.TXT_FONTCOLOR_A: {
            if (indexType === MATERIAL_INDEX_TYPE.TEXT) {
              const fontColor = g_ObjectData.get(objectId - 1).matIndexFontColor[materialIndex];
              const g_DialogInfoRow = `Text\tFont Alpha\t${aRGBtoA(fontColor)}/${0xff}\n`;
              g_DialogInfo += g_DialogInfoRow;
            } else {
      ...
      switch (dialogId) {
        ...
        case DIALOG_ID.COLORALPHA_FONTFACE: {
          ...
          if (
            ...
            const argb = g_ObjectData.get(objectId - 1).matIndexFontColor[materialIndex];
            const rgb = aRGBtoRGB(argb).toString(16).padStart(6, "0");
            g_DialogInfo = `Current Value: 0x${rgb}/${0xff}`;
          } else {
            g_DialogInfo = "Current Value: none";
...
export function findObjects(result: number[], resultSize: number, search: string, offset: number) {
  let rowsFound = 0;
  let rowsAdded = 0;
  let maxOffset = 0;
  ...
  ObjectMp.getInstances().forEach((o) => {
    ...
    if (
      ...
      (cacheIndex !== INVALID_ARRAY_INDEX && g_ModelCache.get(cacheIndex).name.includes(search))
    ) {
      if (rowsFound++ < offset) {
        return;
      }
