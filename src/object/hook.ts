...
import { g_SelectListPTD, g_SelectObjListData, MAX_SELECT_LIST_ROWS } from "@/selectList";
...
function h_CreateObject(
  ...
  const objectId = orig_CreateObject(modelId, x, y, z, rX, rY, rZ, drawDistance);
  if (objectId !== InvalidEnum.OBJECT_ID) {
    ...
    for (let materialIndex = 0; materialIndex < MAX_OBJECT_INDEX; materialIndex++) {
      g_ObjectData.get(objectId - 1).matIndexType[materialIndex] = MATERIAL_INDEX_TYPE.NONE;
    }
  }
  return objectId;
}
...
function h_CreatePlayerObject(
  ...
  const objectId = orig_CreatePlayerObject(playerId, modelId, x, y, z, rX, rY, rZ, drawDistance);
  if (objectId !== InvalidEnum.OBJECT_ID) {
    const g_CommentString = getModelName(modelId);
    if (g_CommentString.ret) {
      g_ObjectData.get(objectId - 1).comment = g_CommentString.name;
...
ObjectMp.__inject__.setMaterial = h_SetObjectMaterial;
...
const orig_SetPlayerObjectMaterialText = ObjectMp.__inject__.setMaterialTextPlayer;
