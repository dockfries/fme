...
import { showObjectDialog, g_ObjectData, applyObjectAttachData } from "@/object";
...
GameMode.onInit(({ next }) => {
  ...
  g_OffsetEditTimer = setInterval(onOffsetEditUpdate, OFFSETEDIT_UPDATE_INTERVAL);
...
function onOffsetEditUpdate() {
  Player.getInstances().forEach((p) => {
    ...
    if (keys & KeysEnum.WALK) {
      ...
      moveAmount = g_OffsetEditData.get(p.id).multiplier * OFFSETEDIT_MOVE_NORMAL;
    }
...
export function showOffsetEditMode(player: Player) {
  const text = `~r~editing ${getOffsetEditModeName(g_OffsetEditData.get(player.id).mode)} offset`;
...
export function toggleOffsetEdit(player: Player, toggle: boolean) {
  ...
  if (toggle) {
    ...
    player.sendClientMessage(RGBA_ORANGE, "Move Slower: {FFFFFF}~k~~SNEAK_ABOUT~ + Direction Key");
    player.sendClientMessage(RGBA_ORANGE, "Change Mode: {FFFFFF}~k~~PED_SPRINT~ + Direction Key");
