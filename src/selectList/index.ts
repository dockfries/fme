...
export const g_SelectObjListData = new SafetyMap<number, ISelectListData>(() => {
  return {
    ...
    search: "",
    rowId: Array.from({ length: MAX_SELECT_LIST_ROWS }, () => InvalidEnum.OBJECT_ID),
    editRow: INVALID_ROW,
    editViewed: false,
  };
});
...
export const g_SelectVehListData = new SafetyMap<number, ISelectListData>(() => {
  return {
    ...
    maxPage: MIN_SELECT_LIST_PAGE,
    search: "",
    rowId: Array.from({ length: MAX_SELECT_LIST_ROWS }, () => InvalidEnum.VEHICLE_ID),
    editRow: INVALID_ROW,
    editViewed: false,
  };
});
...
export const g_SelectPickListData = new SafetyMap<number, ISelectListData>(() => {
  return {
    ...
    maxPage: MIN_SELECT_LIST_PAGE,
    search: "",
    rowId: Array.from({ length: MAX_SELECT_LIST_ROWS }, () => InvalidEnum.PICKUP_ID),
    editRow: INVALID_ROW,
    editViewed: false,
  };
});
...
export const g_SelectActListData = new SafetyMap<number, ISelectListData>(() => {
  return {
    ...
    maxPage: MIN_SELECT_LIST_PAGE,
    search: "",
    rowId: Array.from({ length: MAX_SELECT_LIST_ROWS }, () => InvalidEnum.ACTOR_ID),
    editRow: INVALID_ROW,
    editViewed: false,
  };
});
...
TextDrawEvent.onPlayerClickGlobal(({ player, textDraw, next }) => {
  switch (g_PlayerData.get(player.id).tdMode) {
    ...
    case TD_MODE.SELECTLIST_PICKUP: {
      if (textDraw === g_SelectListGTD.close || textDraw === InvalidEnum.TEXT_DRAW) {
        ...
        case TD_MODE.SELECTLIST_OBJECT: {
          ...
          if (ObjectMp.isValid(attachToObjectId)) {
            ...
            const {
              ...
              y: ySize,
              z: zSize,
            } = Vehicle.getModelInfo(vehicle.getModel(), VehicleModelInfoEnum.SIZE);
...
export function showSelectList(player: Player) {
  ...
  Object.values(g_SelectListGTD).forEach((gtd: ISelectListGtd[keyof ISelectListGtd]) => {
    if (gtd && gtd.isValid()) {
      gtd.show(player);
    }
  });
...
export function getSelectListNewEditRow(player: Player) {
  switch (g_PlayerData.get(player.id).tdMode) {
    ...
    case TD_MODE.SELECTLIST_PICKUP: {
      ...
      if (!Pickup.isValidGlobal(editPickupId)) {
        return INVALID_ROW;
      }
...
export function applySelectListRowColor(player: Player, row: number) {
  ...
  g_SelectListPTD.get(player.id).modelIdRow[row]?.setBoxColors(rgbaBoxColor).show();
  g_SelectListPTD.get(player.id).commentRow[row]?.setBoxColors(rgbaBoxColor).show();
  return 1;
}
...
export function applySelectListRowData(player: Player, row: number) {
  ...
  switch (g_PlayerData.get(player.id).tdMode) {
    ...
    case TD_MODE.SELECTLIST_PICKUP: {
      pickupId = g_SelectPickListData.get(player.id).rowId[row];
      isValid = Pickup.isValidGlobal(pickupId);
      break;
    }
    default: {
      return 0;
  ...
  switch (g_PlayerData.get(player.id).tdMode) {
    ...
    case TD_MODE.SELECTLIST_VEHICLE: {
      ...
      modelIdStr = Vehicle.getInstance(vehicleId)!.getModel().toString();
...
export function loadSelectListRowData(player: Player) {
  ...
  switch (g_PlayerData.get(player.id).tdMode) {
    case TD_MODE.SELECTLIST_OBJECT: {
      ...
      g_SelectObjListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_SELECT_LIST_ROWS);
    ...
    case TD_MODE.SELECTLIST_VEHICLE: {
      ...
      g_SelectVehListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_SELECT_LIST_ROWS);
    ...
    case TD_MODE.SELECTLIST_ACTOR: {
      ...
      g_SelectActListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_SELECT_LIST_ROWS);
    ...
    case TD_MODE.SELECTLIST_PICKUP: {
      ...
      g_SelectPickListData.get(player.id).maxPage = Math.floor(maxOffset / MAX_SELECT_LIST_ROWS);
