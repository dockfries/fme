...
export const g_CategorySelectData = new SafetyMap<number, ICategorySelectData>(() => {
  return {
    ...
    search: "",
    rowId: Array.from({ length: MAX_CATEGORYSELECT_ROWS }, () => INVALID_CATEGORY_ID),
  };
});
...
function onDialogResponse(player: Player, res: IDialogResCommon, dialogId: number) {
  ...
  switch (dialogId) {
    case DIALOG_ID.CATEGORYSELECT: {
      ...
      if (listItem >= LITEM_CATEGORYSELECT.ROW_F && listItem <= LITEM_CATEGORYSELECT.ROW_L) {
        ...
        switch (listItem) {
          ...
          case LITEM_CATEGORYSELECT.PAGE_P: {
            if (--g_CategorySelectData.get(player.id).page < MIN_CATEGORYSELECT_PAGE) {
              g_CategorySelectData.get(player.id).page = MIN_CATEGORYSELECT_PAGE;
            }
            loadCategorySelectData(player);
            break;
          ...
          case LITEM_CATEGORYSELECT.PAGE_L: {
            g_CategorySelectData.get(player.id).page = g_CategorySelectData.get(player.id).maxPage;
            loadCategorySelectData(player);
            break;
          }
    ...
    case DIALOG_ID.CATEGORYSELECT_PAGE: {
      ...
      if (page < MIN_CATEGORYSELECT_PAGE || page > g_CategorySelectData.get(player.id).maxPage) {
        player.sendClientMessage(RGBA_RED, "ERROR: You did not enter a valid page number!");
        showCategorySelectDialog(player, dialogId);
        return 1;
      }
...
export function loadCategorySelectData(player: Player) {
  ...
  g_CategorySelectData.get(player.id).maxPage = Math.floor(maxOffset / MAX_CATEGORYSELECT_ROWS);
  return 1;
}
...
export async function showCategorySelectDialog(player: Player, dialogId: number) {
  switch (dialogId) {
    case DIALOG_ID.CATEGORYSELECT: {
      showCategorySelect(player);
      break;
