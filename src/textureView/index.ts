...
import { GameMode, InvalidEnum, ObjectMp, Player, PlayerEvent } from "@infernus/core";
...
export function destroyTextureView(player: Player) {
  if (ObjectMp.isValid(g_TextureViewData.get(player.id).poId, player.id)) {
    const pObj = ObjectMp.getInstance(g_TextureViewData.get(player.id).poId, player);
    pObj?.destroy();
    g_TextureViewData.get(player.id).poId = InvalidEnum.OBJECT_ID;
  }
}
...
export function refreshTextureView(player: Player, textureId: number) {
  ...
  if (
    g_ObjectData.get(g_TextureViewData.get(player.id).poId - 1).matIndexModCount >=
  ...
  const { modelId, txd: g_TextureTXDString, name: g_TextureNameString } = getTextureData(textureId);

  const { x, y, z } = getTextureViewNextPos(player);

  const pObj = ObjectMp.getInstance(g_TextureViewData.get(player.id).poId, player);

  pObj?.setMaterial(TEXTUREVIEW_MATINDEX, modelId, g_TextureTXDString, g_TextureNameString, 0);
  pObj?.setPos(x, y, z);
  return 1;
}
