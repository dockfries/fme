...
export function isModelCategoryIDCreated(categoryId: number) {
  const g_QueryString = "SELECT * FROM model_category_data WHERE categoryid = ?";
...
export function isTextureCategoryIDCreated(categoryId: number) {
  const g_QueryString = "SELECT * FROM texture_category_data WHERE categoryid = ?";
  const g_DBResult = g_ResourceDB.prepare(g_QueryString).get(categoryId);
  return !!g_DBResult;
}

export function isModelCategoryNameCreated(name: string) {
  const g_QueryString = "SELECT * FROM model_category_data WHERE lower(name) = ?";
  const g_DBResult = g_ResourceDB.prepare(g_QueryString).get(name.toLocaleLowerCase());
  return !!g_DBResult;
}
...
export function isSkinCategoryNameCreated(name: string) {
  const g_QueryString = "SELECT * FROM skin_category_data WHERE lower(name) = ?";
  const g_DBResult = g_ResourceDB.prepare(g_QueryString).get(name.toLocaleLowerCase());
  return !!g_DBResult;
}

export function isTextureCategoryNameCreated(name: string) {
  const g_QueryString = "SELECT * FROM texture_category_data WHERE lower(name) = ?";
  const g_DBResult = g_ResourceDB.prepare(g_QueryString).get(name.toLocaleLowerCase());
  return !!g_DBResult;
}

export function getModelCategoryName(categoryId: number) {
  const g_QueryString = "SELECT name FROM model_category_data WHERE categoryid = ?";
...
export function getVehicleCategoryName(categoryId: number) {
  const g_QueryString = "SELECT name FROM veh_category_data WHERE categoryid = ?";
  const g_DBResult = g_ResourceDB.prepare(g_QueryString).get(categoryId) as
    | { name: string }
    | undefined;
  const categoryFound = !!g_DBResult;
...
export function getSkinCategoryName(categoryId: number) {
  const g_QueryString = "SELECT name FROM skin_category_data WHERE categoryid = ?";
  const g_DBResult = g_ResourceDB.prepare(g_QueryString).get(categoryId) as
    | { name: string }
    | undefined;
  const categoryFound = !!g_DBResult;
...
export function getTextureCategoryName(categoryId: number) {
  const g_QueryString = "SELECT name FROM texture_category_data WHERE categoryid = ?";
  const g_DBResult = g_ResourceDB.prepare(g_QueryString).get(categoryId) as
    | { name: string }
    | undefined;
  const categoryFound = !!g_DBResult;
  ...
  if (!search.trim().length) {
    g_QueryString =
      "\
            ...
            LIMIT ? OFFSET ?";
    g_DBResult = g_ResourceDB.prepare(g_QueryString).all(resultSize, offset) as typeof g_DBResult;
  } else {
    ...
    g_DBResult = g_ResourceDB
      ...
      .all(_search, _search, _search, _search, resultSize, offset) as typeof g_DBResult;
  }
...
export function destroyTextureCategory(categoryId: number) {
  ...
  const g_QueryString = "DELETE FROM texture_category_data WHERE categoryid = ?";
  const info = g_ResourceDB.prepare(g_QueryString).run(categoryId);
  return info.changes === 1;
}
...
export function renameModelCategory(categoryId: number, newName: string) {
  ...
  const g_QueryString = "UPDATE model_category_data SET name = ? WHERE categoryid = ?";
...
export function renameVehicleCategory(categoryId: number, newName: string) {
  ...
  const g_QueryString = "UPDATE veh_category_data SET name = ? WHERE categoryid = ?";
  const info = g_ResourceDB.prepare(g_QueryString).run(newName, categoryId);
  return info.changes === 1;
}
...
export function renameSkinCategory(categoryId: number, newName: string) {
  ...
  const g_QueryString = "UPDATE skin_category_data SET name = ? WHERE categoryid = ?";
  const info = g_ResourceDB.prepare(g_QueryString).run(newName, categoryId);
  return info.changes === 1;
}
...
export function renameTextureCategory(categoryId: number, newName: string) {
  ...
  const g_QueryString = "UPDATE texture_category_data SET name = ? WHERE categoryid = ?";
  const info = g_ResourceDB.prepare(g_QueryString).run(newName, categoryId);
  return info.changes === 1;
}
...
export function getModelCategoryBindID(categoryId: number, modelId: number) {
  ...
  const g_DBResult = g_ResourceDB.prepare(g_QueryString).get(categoryId, modelId) as
    | { bindid: number }
    | undefined;
...
export function getVehicleCategoryBindID(categoryId: number, modelId: number) {
  const g_QueryString = "SELECT bindid FROM veh_category_bind WHERE categoryid = ? AND modelid = ?";
  const g_DBResult = g_ResourceDB.prepare(g_QueryString).get(categoryId, modelId) as
    | { bindid: number }
    | undefined;
  let bindId = INVALID_CATEGORY_BIND_ID;
...
export function getTextureCategoryBindID(categoryId: number, textureId: number) {
  const g_QueryString =
    "SELECT bindid FROM texture_category_bind WHERE categoryid = ? AND textureid = ?";
  const g_DBResult = g_ResourceDB.prepare(g_QueryString).get(categoryId, textureId) as
    | { bindid: number }
    | undefined;
  let bindId = INVALID_CATEGORY_BIND_ID;
  if (g_DBResult) {
...
export function createModelCategoryBind(categoryId: number, modelId: number) {
  ...
  const g_QueryString = "INSERT INTO model_category_bind (categoryid, modelid) VALUES (?, ?)";
...
export function createVehicleCategoryBind(categoryId: number, modelId: number) {
  ...
  const g_QueryString = "INSERT INTO veh_category_bind (categoryid, modelid) VALUES (?, ?)";
  const info = g_ResourceDB.prepare(g_QueryString).run(categoryId, modelId);
  return info.changes === 1;
}
...
export function createSkinCategoryBind(categoryId: number, modelId: number) {
  ...
  const g_QueryString = "INSERT INTO skin_category_bind (categoryid, modelid) VALUES (?, ?)";
  const info = g_ResourceDB.prepare(g_QueryString).run(categoryId, modelId);
  return info.changes === 1;
}

export function createTextureCategoryBind(categoryId: number, textureId: number) {
  if (
  ...
  const g_QueryString = "INSERT INTO texture_category_bind (categoryid, textureid) VALUES (?, ?)";
...
export function destroyVehicleCategoryBind(categoryId: number, modelId: number) {
...
export function destroyTextureCategoryBind(categoryId: number, textureId: number) {
