...
export function loadModelCache() {
  ...
  for (let row = 0, rows = g_DBResult.length; row < rows; row++) {
    if (g_ModelCacheLimit < MAX_MODELS) {
      ...
      g_ModelCache.get(g_ModelCacheLimit).sphereRadius = g_DBResult[row].sphere_radius;
      g_ModelCache.get(g_ModelCacheLimit).sphereOffX = g_DBResult[row].sphere_off_x;
      g_ModelCache.get(g_ModelCacheLimit).sphereOffY = g_DBResult[row].sphere_off_y;
      g_ModelCache.get(g_ModelCacheLimit).sphereOffZ = g_DBResult[row].sphere_off_z;
