...
function onDialogResponse(player: Player, res: IDialogResCommon, dialogId: number) {
  ...
  switch (dialogId) {
    ...
    case DIALOG_ID.ATTACH_COORD: {
      ...
      const cmd = inputText.slice(0, spaceIdx);
      const rest = inputText.slice(spaceIdx + 1).trim();
      if (!cmd || !rest) {
        showAttachedDialog(player, dialogId);
        return 1;
      }
...
export async function showAttachedDialog(player: Player, dialogId: number) {
  switch (dialogId) {
    ...
    case DIALOG_ID.ATTACH_MAIN: {
      ...
      for (let listItem = 0; listItem < DIALOG_LISTITEM_ATTACH.MAX; listItem++) {
        switch (listItem) {
          case DIALOG_LISTITEM_ATTACH.MODEL: {
            if (g_PlayerAttachData.get(player.id)[index].toggle) {
              const modelId = g_PlayerAttachData.get(player.id)[index].model;
          ...
          case DIALOG_LISTITEM_ATTACH.COLOR1: {
            if (g_PlayerAttachData.get(player.id)[index].toggle) {
              const color = aRGBtoRGB(g_PlayerAttachData.get(player.id)[index].color1)
                .toString(16)
                .padStart(6, "0");
          ...
          case DIALOG_LISTITEM_ATTACH.ALPHA1: {
            if (g_PlayerAttachData.get(player.id)[index].toggle) {
              const a = aRGBtoA(g_PlayerAttachData.get(player.id)[index].color1);
          ...
          case DIALOG_LISTITEM_ATTACH.COLOR2: {
            if (g_PlayerAttachData.get(player.id)[index].toggle) {
              const color = aRGBtoRGB(g_PlayerAttachData.get(player.id)[index].color2)
                .toString(16)
                .padStart(6, "0");
          ...
          case DIALOG_LISTITEM_ATTACH.ALPHA2: {
            if (g_PlayerAttachData.get(player.id)[index].toggle) {
              const a = aRGBtoA(g_PlayerAttachData.get(player.id)[index].color2);
    ...
    case DIALOG_ID.ATTACH_COORD: {
      ...
      let g_DialogInfo = "";
      let g_DialogInfoRow = `x \t${g_PlayerAttachData.get(player.id)[index].x}\n`;
      g_DialogInfo += g_DialogInfoRow;
